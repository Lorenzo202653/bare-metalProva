#include "7670inter_prova.h"
#include "string.h"

// La funzione deve essere attivata ad ogni fronte di salita di pclk
void 7670inter_prova(volatile uint8_t data_in, bit href, bit vsync, hls::stream<uint8_t> &data_out, volatile bit *line_valid, volatile bit *frame_valid)
{
	...
	if( vsync == 0 && count_lines < LINES )
	{
		...
		*frame_valid = 1;
		if ( href == 1 )
		{
			data_out.write((uint8_t)data_in);
			//*data_out=data_in;
