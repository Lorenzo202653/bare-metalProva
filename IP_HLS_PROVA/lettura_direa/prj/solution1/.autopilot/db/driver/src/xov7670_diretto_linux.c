...
typedef struct {
    ...
    XOv7670_diretto_uio_map maps[ MAX_UIO_MAPS ];
} XOv7670_diretto_uio_info;

/***************** Variable Definitions **************************************/
static XOv7670_diretto_uio_info uio_info;
...
static int uio_info_read_name(XOv7670_diretto_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/name", info->uio_num);
    return line_from_file(file, info->name);
}

static int uio_info_read_version(XOv7670_diretto_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/version", info->uio_num);
    return line_from_file(file, info->version);
}

static int uio_info_read_map_addr(XOv7670_diretto_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
static int uio_info_read_map_size(XOv7670_diretto_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
int XOv7670_diretto_Initialize(XOv7670_diretto *InstancePtr, const char* InstanceName) {
	XOv7670_diretto_uio_info *InfoPtr = &uio_info;
