...
#define FIRMWARE_LENGTH 50
	...
	{
		printf("[INFO] Command #%d -  Register: 0x%x - Data: 0x%x\n\r", c, firmware[c].Register_address, firmware[c].Register_Data);
	}
...
int OV7670_set_VGA_resolution_and_YUV_output()
{
	...
	//---------------modify CLKRC---------------
	{
		...
		xil_printf("[%s_write_successful]  Register: 0x%x  -  Data: 0x%x\n\r",register_name,register_attuale, data_register_attuale);
	}
...
int OV7670_change_order_for_YUV(char* order)
{
	...
	if(strcmp(order,"YUYV") == 0)
	{
		...
		xil_printf("\n[%s_write_successful]  Register: 0x%x  -  Data: 0x%x\n\r",register_name,register_attuale, data_register_attuale);
...
int OV7670_set_other_registers()
{
	...
	xil_printf("--------end SETTING OTHER REGISTERS--------\n\r");
