...
{
	int status;
	status = XAxi_write_prova_Initialize(&writer, XPAR_XAXIS_TO_DDR_WRITER_0_DEVICE_ID);
...
{
	XAxi_write_prova_Set_base_address(&writer, FRAME_BUFFER_BASE_ADDR);
	XAxi_write_prova_Set_frame_buffer_dim(&writer, FRAME_BUFFER_DIM);
	XAxi_write_prova_Set_frame_buffer_number(&writer, FRAME_BUFFER_NUM);
	XAxi_write_prova_Set_frame_buffer_offset(&writer, FRAME_BUFFER_DIM);
	XAxi_write_prova_EnableAutoRestart(&writer);
	XAxi_write_prova_Set_update_intr(&writer, 1);
	XAxi_write_prova_Start(&writer);
...
u32 getFrameIndex(void){
   uint8_t dato_p=(uint8_t*)( 0x13000000);
   return Xil_In32(dato_p);
   //return XAxi_write_prova_Get_frame_index_V(&writer);
}

void setFrameIndex(void){
	XAxi_write_prova_Set_frame_index_V(&writer,0x13000000);
}
