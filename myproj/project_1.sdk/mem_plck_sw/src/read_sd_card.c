...
static char ImageName2[32] = "0:/image2.pgm";
static FILINFO fno;
static DIR dir;
static char eder[32]="P5\n# feep.pgm\n640 480\n255\n";
BYTE Buffer[FRAME_BUFFER_DIM];
BYTE Buffer2[FRAME_BUFFER_DIM];
//u32 BaseAddress = 0x08000000;
u32 BaseAddress = FRAME_BUFFER_BASE_ADDR;
int Buffer_size = FRAME_BUFFER_DIM;
...
int write_to_sd_card_Test()
{
	...
	sleep(10);
	xil_printf("\n\r");
	Xil_DCacheInvalidateRange(FRAME_BUFFER_BASE_ADDR, 0x96000);
	for (i = 0; i < FRAME_BUFFER_DIM*2 ; i++)
	{
		if (i%2==1) Buffer[i/2]=Xil_In8((BaseAddress) + (i));
		else Buffer2[i/2]=Xil_In8((BaseAddress) + (i));
		if((i%50000)==0)
			xil_printf("Sono ancora qua %d\n\r",i/50000);
	}
	...
	result = f_write(&file, Buffer, FRAME_BUFFER_DIM, &BytesWr);
	if (result != 0)
	{
	...
	result = f_open(&file, ImageName2, FA_CREATE_ALWAYS | FA_WRITE | FA_READ);
		...
		// Write to file
		//result = f_write(&file, (const void*)Buffer, Buffer_size, &BytesWr);
		result = f_write(&file, Buffer2, FRAME_BUFFER_DIM, &BytesWr);
